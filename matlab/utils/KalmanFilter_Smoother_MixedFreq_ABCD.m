function [x, P, loglik,ll, struct_shock, v, K, invF, alpha] = KalmanFilter_Smoother_MixedFreq(y, A, C, Sig_eta, Sig_eps, x_init, P_init,cfg,ABCD)
...
if cfg.flows ;%|| (~cfg.flows && cfg.growth_rates)
   ncol = size(B,2);
    idx = strcmpi(cfg.info.sampling(cfg.info.used_in_estimation), 'flow');
    num_flows = size(cfg.C_original(idx,:),1);
    Ah2 = [expm(Act*(h/2)), zeros(2,num_flows);
          (cfg.C_original(idx,:))*(Act\(expm(Act*(h/2))-eye(size(Act,2))))/(h/2),zeros(num_flows,num_flows)]; %B = B/sqrt(h/2);
...
% STRUCTURALSHOCKS back out structural shocks
    ...
    for t=1:T % recover structural shocks
          epsil(:,t)=  sol * eta(1:size(sol,2),t) ;  
    end
    epsil = epsil/sqrt(h);
    H = H*sqrt(h);
