function [values,vtime] = simulate(g1,impact,T,N,shocks,method,type,blowup,subset)
...
if (method == 'implicit') % EDR -> Geometric series representation of exp(A*dt)
    ...
    switch type
        case 'irf'
            for n = 1 : N
                ...
                values(:,n+1) = gg1*(values(:,n) + (1)*impact*shocks(:,n));
                % values(:,n+1) = gg1\(values(:,n) + (1)*impact*shocks(:,n));
            end
        case 'path'
            for n = 1 : N
                values(:,n+1) = gg1*(values(:,n) + (dt^(1/2))*impact*shocks(:,n)); %Current version
                %values(:,n+1) = gg1*(values(:,n)) + (dt^(1/2))*impact*shocks(:,n); 
                % values(:,n+1) = gg1\(values(:,n) + (dt^(1/2))*impact*shocks(:,n));
            end
    end
...
elseif (method == 'explicit') % Euler-Maruyama
    gg1 = speye(size(g1))+g1*dt;
    if (type == 'irf')
        for n = 1 : N
            values(:,n+1) = gg1*(values(:,n) + (dt^(1/2))*impact*shocks(:,n));
